	...
	this._lineParser = function(line)
	{
		...
		self.cmds.forEach(function(item)
		{
			if (!found)
			{
				...
				if (data)
				{
					if (typeof item.cmdGroup === 'string')
					{
						util.log('\n\n  ' + item.help + '\n\n' + self.usage(item.cmdGroup));
					}
					else
					{
						item.callback(data);
...
CLI.prototype.registerCommands = function (commands)
{
	...
	{
		var cmd = (typeof item.cmdGroup === 'string') ? item.cmdGroup : item.cmd;
		item.compiledExpr = XRegExp('^' + cmd + '$');
		item.name = cmd.split(' ')[0];
	});
	this.cmds = commands;
};

CLI.prototype.registerSection = function (sectionName, commands)
{
	registerCommands
};
